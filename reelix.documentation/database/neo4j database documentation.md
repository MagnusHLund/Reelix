...
A user can have fully watched a movie or only partially.
This will be stored as a percentage.
If the user watched the whole movie, then its likely to be interesting for that user.
...
The user is only allowed to get recommended media which has the same ageRestriction, or below.
...
Recommendations can be based on the date on when it was added.
...
If a user removes a like or dislike, the relationship is removed. So there is no 0.
...
The A watch count which indicates how much media a user has consumed, with the given tag or genre.
...
Total times watched any media which a contributor is part of.
A datetime which indicates when the user last watched media that the contributor is part of.
...
A user should be recommended the language that they prefer to hear/read.
...
All media belongs to a single media library.
...
The datetime can be used to see if its time to fetch new data, in relation to the age restriction.
...
To make it fair, each external rating source has a max rating, which can be used to divide the score into a specific range.
...
The relationship between movies and tags/genres are pretty straight forward.
...
Media have relationships to languages, indicating that they have audio and/or subtitles in a specific language.
